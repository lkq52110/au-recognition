...
def get_dataloader(conf):
    ...
    if conf.dataset == 'DISFA':
        trainset = DISFA(conf.dataset_path, train=True, fold=conf.fold, transform=image_train(crop_size=conf.crop_size),
        ...
        valset = DISFA(conf.dataset_path, train=False, fold=conf.fold, transform=image_test(crop_size=conf.crop_size))
        val_loader = DataLoader(valset, batch_size=conf.batch_size, shuffle=False, num_workers=conf.num_workers)

    elif conf.dataset == 'EmotioNet':
        trainset = EmotioNet(conf.dataset_path, train=True, fold=conf.fold,
                             transform=image_train(crop_size=conf.crop_size),
                             crop_size=conf.crop_size)
        train_loader = DataLoader(trainset, batch_size=conf.batch_size, shuffle=True, num_workers=conf.num_workers)
        valset = EmotioNet(conf.dataset_path, train=False, fold=conf.fold,
...
def main(conf):
    if conf.dataset == 'DISFA':
        ...
        dataset_info = EmotioNet_infolist

    start_epoch = 0
    # data
    train_loader, val_loader, train_data_num, val_data_num = get_dataloader(conf)
    train_weight = torch.from_numpy(np.loadtxt(
        os.path.join(conf.dataset_path, 'list', conf.dataset + '_train_weight_fold' + str(conf.fold) + '.txt')))

    logging.info("Fold: [{} | {}  val_data_num: {} ]".format(conf.fold, conf.N_fold, val_data_num))

    net = MACG(num_classes=conf.num_classes, backbone=conf.arc, neighbor_num=conf.neighbor_num, metric=conf.metric)
    # resume
    if conf.resume != '':
    ...
    print('Init learning rate is ', conf.learning_rate)

    # tensorboard
    # tensorboard --logdir=./outdir/tensorboard_logs
    writer = SummaryWriter(log_dir=os.path.join(conf['outdir'], 'tensorboard_logs'))
    ...
    for epoch in range(start_epoch, conf.epochs):
        ...
        writer.add_scalar('F1_Score/val_mean_f1_score', 100. * val_mean_f1_score, epoch)
        writer.add_scalar('Accuracy/val_mean_acc', 100. * val_mean_acc, epoch)

        if val_mean_f1_score > best_f1_score:
            best_f1_score = val_mean_f1_score
            best_model_path = os.path.join(conf['outdir'], f'best_model_fold{conf.fold}.pth')
            torch.save({'epoch': epoch, 'state_dict': net.state_dict(), 'optimizer': optimizer.state_dict()},
                       best_model_path)
            logging.info(f"Saved new best model with F1-score: {100. * val_mean_f1_score:.2f}% at epoch {epoch + 1}")

        # save checkpoints
        # if epoch < 10 or (epoch >= 10 and (epoch + 1) % 4 == 0):
        if (epoch+1) % 4 == 0:
            checkpoint = {
                'epoch': epoch,
